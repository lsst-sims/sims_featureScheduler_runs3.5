...
def ddf_surveys(
    ...
    if split_filters is not None:
        for filtername in split_filters:
            to_split = np.where(obs_array['filter'] == filtername)
            obs_array['mjd'][to_split] += 1
...
def sched_argparser():
    ...
    parser.add_argument(
        "--maxDither", type=float, default=0.2, help="Dither size for DDFs (deg)"
    )
    parser.add_argument(
        "--moon_illum_limit",
        type=float,
    ...
    parser.set_defaults(no_too=True)
