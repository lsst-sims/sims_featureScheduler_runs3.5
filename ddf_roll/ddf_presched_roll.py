...
def optimize_ddf_times(
    ...
    all_season_int = np.floor(calc_season(ddf_RA, ddf_grid["mjd"], mjd_start))
    ...
    for night_check in nights_to_use:
        ...
        # we could intoepolate this to get even better than 15 min
        # resolution on when to observe
        max_indx = np.where(m5s == m5s.max())[0].min()
        mjds.append(ddf_grid["mjd"][in_night[max_indx]])
        out_season_int.append(all_season_int[in_night[max_indx]])

    return mjds, out_season_int #,night_mjd, cumulative_desired, cumulative_sched, 
...
def generate_ddf_scheduled_obs(
    ...
    roll_fields = {"ELAISS1": {'high': [1, 3, 6], 'low': [2, 4, 7]}, 
                   "XMM_LSS": {"high": [2, 4, 7], "low": [3, 5, 8]}}
    ...
    for ddf_name in ["ELAISS1", "XMM_LSS", "ECDFS", "COSMOS", "EDFS_a"]:
        ...
        mjds, out_season_int = optimize_ddf_times(
            ddf_name,
            ddfs[ddf_name][0],
            ddf_grid,
        ...
        mjds = np.array(mjds)
        # If we are rolling the field, build list season-by-season
        if ddf_name in roll_fields.keys():
            for season_i in np.unique(out_season_int):
                ...
                for mjd in mjds[in_season]:
                    for filtername, nvis, nexp in zip(filters, nvis_now, nsnaps):
